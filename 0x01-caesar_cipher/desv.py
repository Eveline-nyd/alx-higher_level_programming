...
def expand(block: str) -> str:
    """Expand 32-bit block to 48-bit using E table"""
...
def s_box_substitution(block: str) -> str:
    """Apply S-box substitution to 48-bit block"""
    output = ''
    # Split into 8 6-bit chunks
    chunks = [block[i * 6:(i + 1) * 6] for i in range(8)]
    for i in range(8):
        chunk = chunks[i]
...
def feistel_function(r_block: str, round_key: str) -> str:
    """The F function in DES"""
    # Expansion
    expanded = expand(r_block)
    # Key mixing
...
def des_round(left: str, right: str, round_key: str) -> (str, str):
...
def des_encrypt_block(block: str, keys: List[str]) -> str:
...
if __name__ == "__main__":
    # Input should be 64-bit (8 byte) block
    plaintext = "01234567"
    key = "13345779"  # 64-bit key (56 bits + 8 parity bits)

    # Convert to binary strings
    plaintext_bin = bin(int.from_bytes(plaintext.encode('utf-8'),
                                       'big'))[2:].zfill(64)
    key_bin = bin(int.from_bytes(key.encode('utf-8'), 'big'))[2:].zfill(64)

    # Generate round keys (simplified - would use proper key schedule)
    round_keys = [key_bin[:48]] * 16  # In reality, each is different

    # Encrypt
    ciphertext_bin = des_encrypt_block(plaintext_bin, round_keys)
